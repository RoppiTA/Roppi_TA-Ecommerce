            ...
            {filtradas.map((c) => (
              ...
              >
                <div className="space-y-1">
                  <div className="flex items-center gap-2"><span className="font-bold text-sm text-brand-dark">COT-N° {c.id}</span><StatusBadge estado={c.estado} /></div>
                  <div className="flex items-center gap-1 text-xs font-semibold text-brand-muted"><UserCheck className="w-3.5 h-3.5" /> <span>Cliente: {c.cliente}</span></div>
                  <div className="text-[11px] text-brand-muted/70 flex items-center gap-1"><Calendar className="w-3 h-3" /> Solicitado el {c.fechaSolicitud}</div>
                </div>
                <div className="text-right space-y-0.5 shrink-0">
                  <span className="text-sm font-bold text-primary-hover">S/ {c.total.toFixed(2)}</span>
                  <div className="text-[10px] text-brand-muted/70 font-bold flex items-center justify-end gap-1"><Hash className="w-2.5 h-2.5" /> v{c.version} · {c.cantidadProductos} unds.</div>
